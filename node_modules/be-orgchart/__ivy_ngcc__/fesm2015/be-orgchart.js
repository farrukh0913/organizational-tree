import { min, max, zoom, select, zoomIdentity, stratify, tree, event } from 'd3';
...
class D3NodeBasicParser {
    ...
    addComponent(gNode, component) {
        const el = gNode.append(component.type);
        el.attr('class', component.className);
        for (const key in component) {
            switch (key) {
                case 'refField':
                    el.text(d => d.data[component[key]]);
                default:
...
const calcIconGTransform = (prIcon, prScale) => {
    //defining variables
    let traslateX = 0;
    let traslateY = 0;
    let scale = 1;
...
const calcGFit = (prG, prContainer, prRoot, prNodeParser, prPadding = 50) => {
    //defining variables
...
const addDefaultDefs = (container, defs, prNodeParser) => {
    // adding defs
    for (const key in defs) {
        const currentDef = defs[key];
        // creating pattern
...
const addFaIconDefs = (container, defs) => {
    // adding defs
    for (const key in defs) {
        const currentDef = defs[key];
...
const ɵ1 = faIcon;
const setPattern = (prPattern, prNodeData, prNodeParser) => {
    // configurin pattern
...
const buildTree = (prRoot, nodeSize) => {
...
class D3OrgChart {
    constructor(prContainer, prOptions) {
        ...
        this._data = [];
        //#endregion
        this.zoomFunc = zoom().scaleExtent([0.1, 20]).on("zoom", d => this.zoomed());
        this.currentZoom = 1;
        //#region Events
        // node click
    ...
    prepareCanvas() {
        ...
        if (!me.lastTransform)
            me.svg.transition().duration(200).call(me.zoomFunc.transform, zoomIdentity.translate(containerRect.width / 2, this.nodeParser.height).scale(1));
        //Add container g element
    ...
    prepareData() {
        ...
        if (!me.root) {
            try { // preventing multiple root
                me.root = stratify().id(({ nodeId }) => nodeId).parentId(({ parentNodeId }) => parentNodeId)(me.data);
            }
            catch (err) {
                me.root = stratify().id(({ nodeId }) => nodeId).parentId(({ parentNodeId }) => parentNodeId)([{
                        nodeId: 'root',
                        parentNodeId: '',
                        title: 'Error',
    ...
    showNodes(prNode = null) {
        ...
        me.defs.selectAll('pattern.node')
            ...
            // adding pattern
            const pattern = select(enterNodes[i]);
    ...
    zoomed() {
        ...
        if (isNaN(me.lastTransform.k) || isNaN(me.lastTransform.x) || isNaN(me.lastTransform.y))
            return;
        me.chart.attr('transform', event.transform);
    }
    ...
    traslateTo(prX, prY) {
        ...
        me.svg
            .transition().duration(me.nodeParser.transitionDuration)
            .call(me.zoomFunc.transform, zoomIdentity.translate(prX, prY));
    }
    fit() {
        const me = this;
        ...
        me.svg.transition().duration(me.nodeParser.transitionDuration).call(me.zoomFunc.transform, zoomIdentity.translate(fit.x, fit.y).scale(fit.scale));
    }
...
BEOrgchartComponent.ɵfac = function BEOrgchartComponent_Factory(t) { return new (t || BEOrgchartComponent)(ɵngcc0.ɵɵdirectiveInject(ɵngcc0.ElementRef)); };
BEOrgchartComponent.ɵcmp = /*@__PURE__*/ ɵngcc0.ɵɵdefineComponent({ type: BEOrgchartComponent, selectors: [["be-orgchart"]], inputs: { nodes: "nodes", nodeParser: "nodeParser", options: "options" }, outputs: { onNodeClick: "onNodeClick" }, features: [ɵngcc0.ɵɵNgOnChangesFeature], decls: 2, vars: 0, consts: [[1, "container"], ["orgchart", ""]], template: function BEOrgchartComponent_Template(rf, ctx) { if (rf & 1) {
        ɵngcc0.ɵɵelement(0, "div", 0, 1);
    } }, styles: ["[_nghost-%COMP%]{display:flex;flex-direction:column;overflow:hidden}"] });
...
BEOrgchartModule.ɵfac = function BEOrgchartModule_Factory(t) { return new (t || BEOrgchartModule)(); };
BEOrgchartModule.ɵmod = /*@__PURE__*/ ɵngcc0.ɵɵdefineNgModule({ type: BEOrgchartModule });
BEOrgchartModule.ɵinj = /*@__PURE__*/ ɵngcc0.ɵɵdefineInjector({});
(function () { (typeof ngDevMode === "undefined" || ngDevMode) && ɵngcc0.ɵsetClassMetadata(BEOrgchartComponent, [{
        type: Component,
        args: [{
    ...
    }], function () { return [{ type: ɵngcc0.ElementRef }]; }, { onNodeClick: [{
            type: Output
        }], nodes: [{
            type: Input
        }], nodeParser: [{
...
(function () { (typeof ngDevMode === "undefined" || ngDevMode) && ɵngcc0.ɵsetClassMetadata(BEOrgchartModule, [{
        type: NgModule,
        args: [{
...
(function () { (typeof ngJitMode === "undefined" || ngJitMode) && ɵngcc0.ɵɵsetNgModuleScope(BEOrgchartModule, { declarations: [BEOrgchartComponent], exports: [BEOrgchartComponent] }); })();
...
export { BEOrgchartComponent, BEOrgchartModule, D3NodeBasicParser, D3OrgChart, BEOrgchartComponent as ɵa };

//# sourceMappingURL=be-orgchart.js.map